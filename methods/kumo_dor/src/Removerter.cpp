...
void MapUpdater::parseDynamicIdx2PointCloud() {
    ...
    // get pointcloud
    GetPointcloudUsingPtIdx(dynamic_point_indexes_unique, map_arranged_, map_cleaned_, true);   // 移除动态Indices中所有点
    GetPointcloudUsingPtIdx(dynamic_point_indexes_unique, map_arranged_, map_dynamic_, false); // 只保留动态Indices中所有点

...
class CalcPRRR:
    def __init__(self) -> None:
        self.DYNAMIC_CLASSES = [252, 253, 254, 255, 256, 257, 258, 259]
        self.DATASET_NAME = ""
        self.ALGORITHM_NAME = ""

    def set_dynamic_classes(self, dynamic_classes : list):
        self.DYNAMIC_CLASSES = dynamic_classes

    def set_dataset_name(self, dataset_name : str):
        self.DATASET_NAME = dataset_name

    def set_algorithm_name(self, algorithm_name : str):
        self.ALGORITHM_NAME = algorithm_name

    def fetch_dynamic_objects_ids(self, intensity_np):
    ...
    def count_static_and_dynamic(self, intensity_np, verbose=False):
    ...
    def show_num_static_dynamc(self, path):
        print("On loading data...")
        data = pypcd.PointCloud.from_path(path)
        print("Complete to load data")
        self.count_static_and_dynamic(data.pc_data['intensity'])

    def calc_metrics(self, gt, estimate, dists, indices, voxelsize):
        ...
        for gt_sem, est_sem in tqdm(zip(gt_inliers_semantic, est_inliers_semantic)):
            if gt_sem in self.DYNAMIC_CLASSES:  # dynamic
                pass
            else:
                if est_sem in self.DYNAMIC_CLASSES:  # dynamic <-> dynamic
                    pass
                else:  # static <-> static
    ...
    def calc_naive_preservation(self, gt, estimate, dists, indices, voxelsize):
        num_pc = gt.pc_data['x'].shape[0]
        assert num_pc == dists.shape[0]
        assert num_pc == indices.shape[0]
        ...
        for gt_sem, est_sem in tqdm(zip(gt_inliers, est_inliers)):
            if (gt_sem not in self.DYNAMIC_CLASSES) and (est_sem not in self.DYNAMIC_CLASSES):  # both are static
                num_static_preserved += 1

            elif (gt_sem in self.DYNAMIC_CLASSES) and (est_sem in self.DYNAMIC_CLASSES):  # both are dynamic
    ...
    def print_content(self):
        print(tabulate(self.contents, headers=["Class ID", "R. R", "# remain pts", "# all"], tablefmt="github"))


    def evaluate(self, gt, estimate, voxelsize=0.2):
        # 0. print current state
        num_gt = self.count_static_and_dynamic(gt.pc_data['intensity'])
        num_estimate = self.count_static_and_dynamic(estimate.pc_data['intensity'])

        self.contents = []
        # Print class-wise rejection rate
        for dyn_class in self.DYNAMIC_CLASSES:
        ...
        num_preserved, num_static_preserved, num_dynamic_preserved = self.calc_naive_preservation(gt, estimate, np.reshape(dists, -1), np.reshape(indices, -1), voxelsize)
        # Print metrics

        gt_data = [num_gt['static'], num_gt['dynamic'], num_gt['percentage']]
        est_data = [num_estimate['dynamic'], 100.0*num_estimate['dynamic'] / num_gt['dynamic']]
        ...
        printed_data = [self.DATASET_NAME]  +  [self.ALGORITHM_NAME] + gt_data + est_data + [pr, rr, 2 * (pr/100) * (rr/100) / ((pr/100) + (rr/100))]
